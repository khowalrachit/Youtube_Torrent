...
def download_youtube_video(url, output_path='.', file_name=None):
    try:
        ...
        # Determine the file name for the video
        if not file_name:
            file_name = yt.title

        # Set the output path and download the video
        file_path = os.path.join(output_path, f"{file_name}.mp4")
...
def main(delete_temp_dir=True):
    while True:
        convert_option = input("Enter 0 to download the list of avaliable files\nElse enter the Index of file you want to download\nYour choice: ")
        # input validation
        if convert_option == "0":
            # Add the functionality of 0
            # Add the functionality of deleting abc file
            # Add the functionality of renaming the final file to desired file
            # Delete the garbage code
            ...
            ####################
            ####################            
            request_data = spreadsheet.cell(convert_option,'3').value
            video_url = request_data
            desired_file_name = "abc.mp4"
            download_youtube_video(video_url, file_name= desired_file_name)
